...
def draw_bbox(image, bboxes, info = False, counted_classes = None, show_label=True, allowed_classes=list(read_class_names(cfg.YOLO.CLASSES).values()), read_plate = False):
    ...
    print("the number of boxes is: " + str(num_boxes))
    for i in range(num_boxes):
        if int(out_classes[i]) == 0: continue
        coor = out_boxes[i]
        fontScale = 3
        score = out_scores[i]
        ...
        if class_name not in allowed_classes:
            ...
            if read_plate:
                ...
                if int(out_classes[i]) == 1:
                    plate_number = recognize_kwh(image, coor)
                else:
                    plate_number = recognize_id(image, coor)
